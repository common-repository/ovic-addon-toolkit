 ...
 *
 */
if (!class_exists('OVIC_Comment')) {
    class OVIC_Comment extends OVIC_Abstract
    {
        // constants
        ...
        {
            // Get options metabox
            $this->args    = apply_filters('ovic_comment_metabox_settings', $this->args, $this);
            $this->options = apply_filters('ovic_comment_metabox', $options);
        ...
        public function add_meta_box_content($comment, $callback)
        {
            ...
            wp_nonce_field('ovic-comment', 'ovic-comment-nonce');
            ...
            do_action('ovic_html_comment_before');
            ...
            do_action('ovic_html_comment_after');
        }

        // save metabox
        public function save_meta_box($post_id, $post)
        {
            if (wp_verify_nonce(ovic_get_var('ovic-comment-nonce'), 'ovic-comment')) {
                $errors = array();
                foreach ($this->options as $request_value) {
                    ...
                    $request = apply_filters('ovic_save_comment_metabox', $request, $request_key, $post);
